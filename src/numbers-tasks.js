...
function getRectangleArea(fWidth, fHeight) {
 ...
 *   0    => 0
 */
function getCircleCircumference(fRadius) {
  return 2 * Math.PI * fRadius;
}
 ...
 *  -3, 3  => 0
 */
function getAverage(fValue1, fValue2) {
  return fValue1 / 2 + fValue2 / 2;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  const dVect = x1 * x2 + y1 * y2;
  const dVect1 = Math.sqrt(x1 * x1 + y1 * y1);
  const dVect2 = Math.sqrt(x2 * x2 + y2 * y2);
  return Math.acos(dVect / (dVect1 * dVect2));
}
...
function roundToPowerOfTen(num, pow) {
  const fDivision = 10 ** pow;
  return Math.round(num / fDivision) * fDivision;
}
...
function getSumOfNumbers(x1, x2, x3) {
  const fSum = (x1 * 1000 + x2 * 1000 + x3 * 1000) / 1000;
  return fSum;
}
...
function getCountOfOddNumbers(number) {
  let cOdd = 0;
  const fNum = Math.abs(number);
  for (let i = 0; i <= fNum; i += 1) {
    if (i % 2 === 1) {
      cOdd += 1;
    }
  }
